...
public class AddShow extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		s.setScreen(Integer.parseInt(request.getParameter("screenno")));
		s.setSeat(Integer.parseInt(request.getParameter("seatno")));
		s.setStrttime(request.getParameter("starttm"));
		s.setEndtime(request.getParameter("endtm"));
		...
		int status=ShowDao.save(s);
		System.out.println(status);
		if(status>0)
		{
			response.sendRedirect("addshow.jsp?msg=Show added");
		}
		else
		{
			response.sendRedirect("addshow.jsp?msg=you have already added this show");
		}
	}
}
